...
def main():
    parser = argparse.ArgumentParser("ip_probes.py")
    ...
    base_ip = args.ip_address if args.ip_address else BASE_IP
    target_ips = args.target.split('-') if args.target else ["1"]
    if len(target_ips) >= 1:
        start_ip = int(target_ips[0])
        end_ip = int(target_ips[1]) if len(target_ips) > 1 else start_ip
    count = args.count if args.count else 30
    target_range = args.target_range.split('-') if args.target_range else ["1", "6"]
